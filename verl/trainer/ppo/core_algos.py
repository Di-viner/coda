...
def compute_grpo_outcome_advantage(
    ...
    group_correct_mean_lens = []
    group_wrong_mean_lens = []

    easy_trigger_count = 0
    hard_trigger_count = 0

    with torch.no_grad():
        # Calculate valid response length
        valid_response_length = response_mask.sum(dim=-1).float()
        
        bsz = scores.shape[0]
        for i in range(bsz):
            id2score[index[i]].append(scores[i])
            id2len[index[i]].append(valid_response_length[i])
            
        total_groups = len(id2score)
        
        for idx in id2score:
            lens_tensor = torch.stack(id2len[idx])
            scores_tensor = torch.stack(id2score[idx])

            # Metrics: assume score >= 1.0 is correct (binary outcome)
            is_correct = scores_tensor >= 1.0

            success_ratio = is_correct.sum() / len(is_correct)
            if success_ratio > 0.8:
                easy_trigger_count += 1
            elif success_ratio < 0.2:
                hard_trigger_count += 1

            if is_correct.any() and (~is_correct).any():
                group_correct_mean_lens.append(lens_tensor[is_correct].float().mean().item())
                group_wrong_mean_lens.append(lens_tensor[~is_correct].float().mean().item())
    ...
    metrics = {
        "grpo/mean_group_response_length_correct": np.mean(group_correct_mean_lens) if group_correct_mean_lens else 0.0,
        "grpo/mean_group_response_length_wrong": np.mean(group_wrong_mean_lens) if group_wrong_mean_lens else 0.0,
        "grpo/easy_condition_trigger_rate": easy_trigger_count / total_groups if total_groups > 0 else 0.0,
        "grpo/hard_condition_trigger_rate": hard_trigger_count / total_groups if total_groups > 0 else 0.0,
    }
...
@register_adv_est("coda")
def compute_coda_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,
    response_str: Any = None,
    prompt_str: Any = None,
    valid_response_length: Any = None,
    valid_prompt_length: Any = None,
    config: Optional[AlgoConfig] = None,
    ...
    # Coefficients for penalty/reward
    alpha = 0.2
    beta = 0.2


    if hasattr(config, "coda") and config.coda is not None:
        cfg = config.coda
        success_threshold_easy = cfg.get("success_threshold_easy", success_threshold_easy)
        success_threshold_hard = cfg.get("success_threshold_hard", success_threshold_hard)
        alpha = cfg.get("easy_len_penalty_coef", alpha) 
        beta = cfg.get("hard_len_reward_coef", beta)
    ...
    if isinstance(valid_response_length, np.ndarray):
        valid_response_length = torch.from_numpy(valid_response_length).to(device)
    
    # Identify correct/wrong based on score >= 1.0 (Assuming binary outcome 0/1)
    is_correct = (scores >= 1.0)
    ...
    w_easy_list = []
    w_hard_list = []
    # Global per-sample adjustments (include zeros for untouched samples)
    easy_penalty_per_sample = torch.zeros_like(scores)
    hard_reward_per_sample = torch.zeros_like(scores)
    ...
    easy_trigger_count = 0
    hard_trigger_count = 0
    hard_trigger_no_correct_count = 0
    easy_trigger_all_correct_count = 0
    total_groups = len(id2indices)

    for idx, sample_indices in id2indices.items():
        indices_tensor = torch.tensor(sample_indices, device=device)
        group_scores = scores[indices_tensor]
        
        # Success Rate
        success_rate = (group_scores >= 1.0).float().mean()
        success_rates.append(success_rate.item())

        # Calculate metrics for mixed groups
        group_is_correct = is_correct[indices_tensor]
        group_is_wrong = ~group_is_correct
        correct_indices = indices_tensor[group_is_correct]
        wrong_indices = indices_tensor[group_is_wrong]
        
        if len(correct_indices) > 0 and len(wrong_indices) > 0:
            c_lens = valid_response_length[correct_indices].float()
            w_lens = valid_response_length[wrong_indices].float()
            group_correct_mean_lens.append(c_lens.mean().item())
            group_wrong_mean_lens.append(w_lens.mean().item())
        
        # 1. Easy Task Logic: Only if success_rate > success_threshold_easy
        # Apply Length Penalty to Correct answers
        if success_rate >= success_threshold_easy:
            easy_trigger_count += 1
            
            dist_to_max = 1.0 - success_threshold_easy
            if dist_to_max < 1e-4: dist_to_max = 1.0 # Safety

            w_easy = (success_rate - success_threshold_easy) / dist_to_max
            w_easy = torch.clamp(w_easy, min=0.0, max=1.0)

            w_easy_list.append(w_easy.item())
            
            if len(correct_indices) > 0:
                correct_lens = valid_response_length[correct_indices].float()
                
                if len(correct_indices) > 1:
                    mean_len = correct_lens.mean()
                    std_len = correct_lens.std() + epsilon
                else:
                    mean_len = correct_lens[0]
                    std_len = torch.tensor(1.0, device=device)
                
                z_scores_correct = (valid_response_length[indices_tensor] - mean_len) / std_len
                
                f_val_correct = torch.sigmoid(z_scores_correct)
                
                # Penalty: r_new = r * (1 - w * alpha * f)
                penalty_factor = 1.0 - (w_easy * alpha * f_val_correct)
                assert penalty_factor.min() == 1.0 if alpha == 0.0 else True
                
                current_scores = new_scores[indices_tensor]
                mask_in_group = group_is_correct
                
                new_scores[indices_tensor[mask_in_group]] = current_scores[mask_in_group] * penalty_factor[mask_in_group]
                easy_penalty_amount = current_scores[mask_in_group] * (w_easy * alpha * f_val_correct[mask_in_group])
                easy_penalty_per_sample[indices_tensor[mask_in_group]] += easy_penalty_amount
                
                if w_easy > 0.01:
                     mean_easy_penalty.append(easy_penalty_amount.mean().item())
                if len(wrong_indices) == 0:
                    easy_trigger_all_correct_count += 1
        else:
            w_easy_list.append(0.0)

        # 2. Hard Task Logic: Only if success_rate < success_threshold_hard
        # Apply Length Reward to CORRECT answers (Modified for v5)
        if success_rate <= success_threshold_hard:
            hard_trigger_count += 1
            
            dist_to_min = success_threshold_hard
            if dist_to_min < 1e-4: dist_to_min = 1.0
            
            w_hard = (success_threshold_hard - success_rate) / dist_to_min
            w_hard = torch.clamp(w_hard, min=0.0, max=1.0)


            w_hard_list.append(w_hard.item())
            ...
            if len(correct_indices) > 0:
                ...
                # Reward: r_new = r + w * beta * f
                # Apply to CORRECT answers
                reward_add = w_hard * beta * f_val_correct
                
                current_scores = new_scores[indices_tensor]
                mask_in_group = group_is_correct
                
                new_scores[indices_tensor[mask_in_group]] = current_scores[mask_in_group] + reward_add[mask_in_group]
                hard_reward_per_sample[indices_tensor[mask_in_group]] += reward_add[mask_in_group]
                
                if w_hard > 0.01:
                    mean_hard_reward.append(reward_add[mask_in_group].mean().item())
            else:
                hard_trigger_no_correct_count += 1
        else:
            w_hard_list.append(0.0)

    # GRPO Normalization
    final_scores = new_scores.clone()
    
    for idx, sample_indices in id2indices.items():
        indices_tensor = torch.tensor(sample_indices, device=device)
        g_scores = final_scores[indices_tensor]
        ...
        if norm_adv_by_std_in_grpo:
            final_scores[indices_tensor] = (g_scores - g_mean) / (g_std + epsilon)
        else:
            final_scores[indices_tensor] = g_scores - g_mean
            
    advantages = final_scores.unsqueeze(-1) * response_mask

    # Calculate response lengths for correct/wrong
    correct_lens = valid_response_length[is_correct].float()
    wrong_lens = valid_response_length[~is_correct].float()

    metrics = {
        "coda/mean_score": scores.mean().item(),
        "coda/mean_new_score": new_scores.mean().item(),
        "coda/mean_easy_len_penalty": np.mean(mean_easy_penalty) if mean_easy_penalty else 0.0,
        "coda/mean_hard_len_reward": np.mean(mean_hard_reward) if mean_hard_reward else 0.0,
        "coda/global_easy_penalty_mean": easy_penalty_per_sample.mean().item(),
        "coda/global_hard_reward_mean": hard_reward_per_sample.mean().item(),
        "coda/success_rate": np.mean(success_rates) if success_rates else 0.0,
        "coda/mean_response_length": valid_response_length.float().mean().item(),
        "coda/mean_w_easy": np.mean(w_easy_list) if w_easy_list else 0.0,
        "coda/mean_w_hard": np.mean(w_hard_list) if w_hard_list else 0.0,
        "coda/mean_response_length_correct": correct_lens.mean().item() if len(correct_lens) > 0 else 0.0,
        "coda/mean_response_length_wrong": wrong_lens.mean().item() if len(wrong_lens) > 0 else 0.0,
        "coda/mean_group_response_length_correct": np.mean(group_correct_mean_lens) if group_correct_mean_lens else 0.0,
        "coda/mean_group_response_length_wrong": np.mean(group_wrong_mean_lens) if group_wrong_mean_lens else 0.0,
        "coda/easy_condition_trigger_rate": easy_trigger_count / total_groups if total_groups > 0 else 0.0,
        "coda/easy_trigger_all_correct_rate": easy_trigger_all_correct_count / easy_trigger_count if easy_trigger_count > 0 else 0.0,
        "coda/hard_condition_trigger_rate": hard_trigger_count / total_groups if total_groups > 0 else 0.0,
        "coda/hard_trigger_no_correct_rate": hard_trigger_no_correct_count / hard_trigger_count if hard_trigger_count > 0 else 0.0,
    }
