...
os.environ['ENABLE_REMOTE_DEBUG'] = "1"
...
def main(config):
    ...
    # to json
    # 如果没有对应的json文件，则转换为json
    if not os.path.exists(config.data.path.replace(".parquet", ".json")):
        dataset.to_json(config.data.path.replace(".parquet", ".json"), orient="records", lines=True)
    ...
    if not ray.is_initialized():
        ...
        if config.ray_kwargs.get("enable_debug", True):
